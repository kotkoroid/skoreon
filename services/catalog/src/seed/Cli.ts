...
const databaseId = process.env['CATALOG_D1_DATABASE_ID'] ?? process.argv[2];

if (accountId === undefined || token === undefined || databaseId === undefined) {
  throw new Error(
    'Set CLOUDFLARE_ACCOUNT_ID, CLOUDFLARE_API_TOKEN, and CATALOG_D1_DATABASE_ID (or pass the database id as the first argument).',
  );
}
...
await Effect.runPromise(seedCatalog(db));
